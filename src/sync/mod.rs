...
//! - Save flow: `save_async`'s success branch chains an upload via
//!   `SyncService::after_local_save`, with a one-in-flight + dirty-on-burst
//!   guard so rapid edits collapse into the latest write.
